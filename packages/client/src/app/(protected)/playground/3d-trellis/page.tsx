...
export default function Trellis3dPage() {
  const [removeBg, setRemoveBg] = useState(true);

  const mutation = trpc.ml.generate3dTrellis.useMutation();
    ...
    <div className="space-y-6">
      <div className="flex items-end gap-4">
        <div className="flex-1">
          <FileDropzone
            ...
            maxSizeMb={20}
            onFile={handleFile}
            label="Drop a product photo for TRELLIS 2 (sparse-voxel geometry)"
      ...
      {mutation.isPending && (
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          Generating with TRELLIS 2... (~45 seconds on A100)
        </div>
      )}
