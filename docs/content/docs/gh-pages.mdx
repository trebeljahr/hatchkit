...
- Projects that need a server. Websockets, auth sessions, MongoDB / Postgres, server-rendered routes. Use `hatchkit create` with the full-stack flow — that ends up on Coolify, not Pages.
