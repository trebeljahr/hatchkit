...
6. Put that client id in Hatchkit's packaged Google Search Console client-id
   constant before release. Dev builds can use
   `HATCHKIT_GOOGLE_SEARCH_CONSOLE_CLIENT_ID` to test the same path. Ignore the
   displayed client secret for the packaged PKCE flow.
