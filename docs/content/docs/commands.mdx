...
| Command | Effect |
|---|---|
| `dev-setup init` | Writes `~/.config/dev/Caddyfile`, writes/loads the launchd Caddy job, registers `tailscale serve --tcp=443`, and upserts `*.local.<your-domain> A <tailnet-ip>` when Cloudflare credentials are available. |
