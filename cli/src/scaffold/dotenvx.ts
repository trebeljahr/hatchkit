...
/** Values that hatchkit fills in itself rather than asking the user.
 *  Two flavours:
 *    · Cryptographically random secrets we just mint
 *      (BETTER_AUTH_SECRET, AUTH_SECRET).
 *    · Values fully derivable from data we already know
 *      (FRONTEND_URL, BETTER_AUTH_URL, AUTH_URL — all from `domain`).
 *  Anything in here gets encrypted into .env.production by default,
 *  no CHANGE_ME placeholder, no prompt. */
function autoGeneratedDefaults(config: ProjectConfig): DotenvxValues {
  const secret = randomBytes(32).toString("hex");
  return {
    BETTER_AUTH_SECRET: secret,
    // The starter's `config.ts` historically references `AUTH_SECRET`
    // alongside `BETTER_AUTH_SECRET` — populate both with the same
    // random value so neither is a `change-me` at runtime.
    AUTH_SECRET: secret,
    // Single-domain routing: the bare domain serves the SPA and the
    // API mounts under `/api`. Both env vars use the bare URL — no
    // separate api.<domain> subdomain is needed.
...
export async function seedDotenvxProduction(
  ...
  const defaults = autoGeneratedDefaults(config);
 ...
 *
 *  Anything also returned by `autoGeneratedDefaults` is encrypted
 *  with a real value; keys NOT in that map fall back to a plaintext
 *  CHANGE_ME_<KEY> placeholder so deploys fail loudly instead of
 *  silently running with empty secrets. */
function candidateKeys(config: ProjectConfig): string[] {
  // URLs + secrets are auto-filled — see autoGeneratedDefaults.
  const base = ["BETTER_AUTH_SECRET", "AUTH_SECRET", "BETTER_AUTH_URL", "AUTH_URL", "FRONTEND_URL"];
  // The DB URI is only seeded as a CHANGE_ME placeholder when the user
  // is bringing their own DB — for the Coolify path, hatchkit
  ...
  if (dbProvider !== "coolify") {
    const uriKey = config.dbEngine === "postgres" ? "POSTGRES_URL" : "MONGODB_URI";
    base.unshift(uriKey);
  }
