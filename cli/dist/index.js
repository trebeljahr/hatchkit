...
import { isFirstRun, runOnboarding, getConfig, getConfigPath, ensureCoolify, ensureHetzner, ensureDns, ensureS3, getMlServices, } from "./config.js";
...
// Resolve the dev-ops-automation repo root (parent of cli/)
...
async function handleCreate() {
    ...
    scaffoldInfra(config, REPO_ROOT);
    if (config.dryRun) {
    ...
    if (config.runDeployment) {
        await runTerraform(config, REPO_ROOT);
    }
    // Step 5: Coolify setup
    if (config.runDeployment) {
        await runCoolifySetup(config, REPO_ROOT);
    }
    // Step 6: Deploy ML services
    if (config.runDeployment && deploy.length > 0 && config.gpuPlatform) {
        const endpoints = await deployMlServices(deploy, config.gpuPlatform, REPO_ROOT, config.customHfModelId);
    ...
    switch (subcommand) {
        case "add": {
            ...
            switch (provider) {
                ...
                default:
                    await (await import("./config.js")).ensureGpuProvider(provider);
            }
            break;
        }
        case "reset":
