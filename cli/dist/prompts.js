...
import { getConfig, getMlServices, } from "./config.js";
// ---------------------------------------------------------------------------
// Main prompt flow
// ---------------------------------------------------------------------------
export async function collectProjectConfig(options) {
    ...
    // ML services
    const mlServices = await checkbox({
        message: "ML services:",
        choices: [
...
async function selectExistingServer() {
    ...
    const coolifyConfig = config.providers.coolify;
